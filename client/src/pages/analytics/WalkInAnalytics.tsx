...
export default function WalkInAnalytics() {
  const [search, setSearch] = useState("");
  const { data, isLoading, refetch } = useQuery({
    queryKey: ["analytics-walkin"],
    queryFn: async () => {
  ...
  const handleExport = () => {
    ...
    exportCSV(
      ...
      `walkin-report-${new Date().toISOString().slice(0, 10)}.csv`
    );
  };
  ...
  return (
    ...
    >
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard label="Total Orders" value={data?.totalOrders || 0} icon={<ShoppingCart className="w-4 h-4" />} color="emerald" />
            ...
            {data?.peakHours?.slice(0, 12).map((h: any) => {
              ...
              return (
                <div key={h.hour} className="flex items-center gap-3 text-sm">
                  ...
                  <div className="flex-1"><MiniBar value={h.count} max={max} color="bg-emerald-500" /></div>
