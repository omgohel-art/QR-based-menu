...
export default function OtherAnalytics() {
  ...
  const handleExport = () => {
    ...
    exportCSV(
      ...
      `other-report-${new Date().toISOString().slice(0, 10)}.csv`
    );
  };
  ...
  return (
    ...
    >
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard label="Total Orders" value={data?.totalOrders || 0} icon={<ShoppingCart className="w-4 h-4" />} color="slate" />
            ...
            {data?.peakHours?.slice(0, 12).map((h: any) => {
              ...
              return (
                <div key={h.hour} className="flex items-center gap-3 text-sm">
                  ...
                  <div className="flex-1"><MiniBar value={h.count} max={max} color="bg-slate-500" /></div>
            ...
            {data?.statusBreakdown?.map((s: any) => {
              ...
              return (
                <div key={s.status} className="space-y-1">
                  ...
                  <MiniBar value={s.count} max={max} color="bg-slate-500" />
                </div>
              );
            })}
          </div>
