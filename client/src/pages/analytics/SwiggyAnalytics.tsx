...
export default function SwiggyAnalytics() {
  const [search, setSearch] = useState("");
  const { data, isLoading, refetch } = useQuery({
    queryKey: ["analytics-swiggy"],
    queryFn: async () => {
      ...
      const { data: orders } = await supabase
        .from("orders")
        .select("id, orderStatus, paymentMethod, paymentStatus, submittedAt, finalTotal")
        .eq("orderSource", "swiggy")
      ...
      const totalRevenue = orders.reduce((sum, o) => sum + (parseFloat(o.finalTotal?.toString() || "0") || 0), 0);
  ...
  const handleExport = () => {
    ...
    exportCSV(
      ...
      `swiggy-report-${new Date().toISOString().slice(0, 10)}.csv`
    );
  };
  ...
  return (
    ...
    >
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard label="Total Orders" value={data?.totalOrders || 0} icon={<ShoppingCart className="w-4 h-4" />} color="orange" />
            ...
            {data?.peakHours?.slice(0, 12).map((h: any) => {
              ...
              return (
                <div key={h.hour} className="flex items-center gap-3 text-sm">
                  ...
                  <div className="flex-1"><MiniBar value={h.count} max={max} color="bg-orange-500" /></div>
            ...
            {data?.statusBreakdown?.map((s: any) => {
              ...
              return (
                <div key={s.status} className="space-y-1">
                  ...
                  <MiniBar value={s.count} max={max} color="bg-orange-500" />
                </div>
              );
            })}
          </div>
      ...
      <Card className="p-5 bg-white dark:bg-slate-900">
        ...
        <DataTable
          headers={["Date", "Orders"]}
          rows={data?.peakDays?.filter((d: any) => !search || d.date.includes(search))?.map((d: any) => [d.date, d.count]) || []}
          empty="No order data"
        />
      </Card>
